...
filename = '../glove.6B/glove.6B.300d.txt' #sys.argv[1]
...
def encode_subspace(l1, l2, i):
    ...
    q1 =  np.column_stack(tuple([Glove[x] if x in Glove else np.zeros(300) for i, x in enumerate(l1.split())]))
    q2 = np.column_stack(tuple([Glove[x] if x in Glove else np.zeros(300) for i, x in enumerate(l2.split())]))
        ...
        for line in csv.reader(tsv, dialect="excel-tab"): 
            if skip:
                ...
                if line[1]!='' and line[1]!='':
                    q1, q2 = encode_bow(line[1], line[2], i)
...
print("Accuracy (train) = {}".format(accuracy_test * 100))
